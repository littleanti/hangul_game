...
| ~~확장 세트 구현~~ | **완료 (2026-06-11)** | T3로 해소 — words.js 6→100개(도입 6 불변 + 확장 94) 확장. 구 후보어(봄비·들꽃·눈밭·봄볕)는 확장 세트에 흡수(PRD §7.3) |
...
- 2026-06-11: **T3 단어 100개 확장(문서 선갱신)** — 콘텐츠를 "도입 세트 6개(기존, 불변) + 확장 세트 94개 = 총 100개" 체계로 개편. PRD §7 확장 세트 선정 기준 4종(①순우리말 투명 합성어 ②만 6~7세 수용 어휘 ③두 조각 이모지 표현 가능 ④한자어·파생어·굴절형 배제) 명문화, 구 §7.3 후보 표(봄비·들꽃·눈밭·봄볕) 흡수. TRD §3.1 스키마 갱신: `sharedMorpheme` null 허용(공유 짝 없는 단어), `part1Surface`(사이시옷 표면형) 공식 필드화, `category` 확장분 주제 카테고리(`nature_*`·`body_*`·`season_*`·`food_*` 등) 신규 부여 허용, `SHARED_MORPHEME_PAIRS`를 `{ 형태소: [id, ...] }` 2개 이상 그룹으로 확장. 기술 부채 "확장 세트 구현" 완료 처리
