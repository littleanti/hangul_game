...
> 목표: 경계 가시성·뜻 단서량만 조절하는 3단 페이딩 완전 구현. 입력·화면·어휘는 레벨 간 불변.
...
| R2 | 설계 | L3 승급 조건 — 자동 승급(연속 정답 N회) vs 수동 선택 PRD 미확정 | M3 차단 가능성 | `config.js` 플래그로 양쪽 구현 후 A/B 검증; 기본값은 수동 선택 |
