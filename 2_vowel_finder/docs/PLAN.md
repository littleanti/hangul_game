...
- [ ] `.btn.big` "다시 하기" 버튼 (게임 간 이동 버튼 없음 — 시리즈 정책)
...
- [ ] 정답률 ≥ 80% 시 축하 메시지 / 미달 시 반복 권장 메시지 (SRL 게이팅 미구현 — 메시지로 대체)

> 게임 간 이동(다음 단계 CTA·가로 전환 예고)은 시리즈 정책으로 제거됨 — 아래 "시리즈 내비게이션 제거" 참조.

---

## ✅ 시리즈 내비게이션 제거 (2026-06-12)

게임 간 직접 이동 버튼을 두지 않는 시리즈 정책에 따라 완료 화면의 Stage 2 이동 요소를 제거. 게임 선택은 런처(루트 `index.html`)가 전담한다.

- [x] `index.html` — "음절 조립소로 가기 ➡️" 버튼(`#next-stage-btn`) 및 "다음 게임은 가로 화면을 써요!" 가로 전환 예고 1줄 제거
- [x] `main.js` — `goToNextStage()` 스텁(no-op) 및 전역 노출 제거
- [x] `onboarding.js` — 통과 시 CTA 강조(`cta-glow`) 로직 제거, 완료 메시지를 게임 내 완결형으로 변경
- [x] `screens.css` — `.btn.cta-glow`(+`ctaPulse` 키프레임)·`.rotate-notice` 스타일 제거
- [x] 문서 정합화 — PRD(§6.1·§8 지표·§9 연속성), PLAN 체크리스트

> 학습 설계상의 Stage 2 연속성(출구 기술·드래그 반경·네임스페이스)은 유지 — 제거된 것은 화면 내 이동 UI뿐.
