...
- [x] `.hint-label` position absolute, Jua 0.9rem, `--mint` 배경, 100px border-radius — 자기 음절 위 정중앙, 기본 한 줄(아랫행 -26px) 정렬 + 인접 칩 실측 겹침 시에만 `.staggered`(윗행 -58px) 지그재그
...
게임 간 직접 이동 버튼을 두지 않는 시리즈 정책에 따라 완료 화면의 "다음 단계" 이동 요소를 제거. 게임 선택은 런처(루트 `index.html`)가 전담한다.

- [x] `index.html` — 완료 화면 "다음 단계 🌳" `.btn.mint` 버튼 제거
- [x] `main.js` — `goNextStage()` (`../8_vocabulary_tree/` 이동) 전역 노출 제거
- [x] 문서 정합화 — TRD(§2.3 상태 머신·end-screen 스펙), PRD(§ 화면 인벤토리), PLAN(수동 테스트 체크리스트)

> M2~M3의 "다음 단계" 구현 기록([x] 항목)은 이력으로 보존. `8_vocabulary_tree`와의 학습 설계 연속성(졸업 어휘 핸드오프·난이도 계단)은 유지 — 제거된 것은 화면 내 이동 UI뿐.
