...
> 중심을 보존한다.
>
> **뜻 라벨 칩 겹침 방지:** 칩은 항상 자기 음절 위 **정중앙**(매핑 신호)에 두고, 기본은
> 모든 칩을 **한 줄(아랫행, top: -26px)**에 정렬한다 — 좁은 칩("산 산"/"물 수")까지
> 무조건 지그재그하면 높이가 어긋나 보이기 때문. 칩("시장·도시 시" 등 최대 7자)이
> 세그먼트(음절 폭)보다 넓어 인접 칩과 **실제로 겹치는 경우에만** hint.js 가 렌더 후
> getBoundingClientRect 로 검출해 앞 칩에 `.staggered`(윗행 -58px)를 붙인다.
...
```
L1 상태 (기본 — 칩이 겹치지 않으면 한 줄 정렬):
  [불 화] [산 산]            ← 뜻 라벨 배지 아랫행 한 줄 (각 음절 위 정중앙, --mint 배경, Jua 0.9rem)
  ┌───────┬───────┐
  │  화   │  산   │          ← 색 하이라이트 (--hint-l1-bg, 테두리 --hint-l1-border)
  └───────┴───────┘

L1 상태 (넓은 칩이 인접 칩과 겹칠 때만 — .staggered 지그재그):
  [시장·도시 시]             ← 앞 칩만 윗행으로 (hint.js 가 겹침 실측 후 부여)
        [마당 장]
  ┌───────┬───────┐
  │  시   │  장   │
  └───────┴───────┘
...
.hint-label.staggered {
  top: -58px;                      /* 윗행 — 인접 칩과 실측 겹침 시에만 hint.js 가 부여 */
}
```
