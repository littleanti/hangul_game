...
├── service-worker.js           # AUTO-GENERATED by scripts/gen-sw.mjs
├── docs/                       # PRD/TRD/PLAN
├── scripts/
│   ├── gen-hanja-json.mjs      # hanja.js → src/assets/hanja/*.json 100개
│   ├── gen-vocab.mjs           # hanja.js + stages.js → src/data/vocab.js (429어휘)
│   ├── gen-stage-svg.mjs       # stages.js → src/assets/stages/*.svg 10장
│   ├── gen-sw.mjs              # 자산 list 동기화 + cache version bump
│   ├── validate-data.js        # 데이터 정합성 검증 (`npm run validate`)
│   └── gen-icons.mjs           # PWA 아이콘
└── src/
    ├── css/
